...
import { Archive, BrainCircuit, FlaskConical, Play, RefreshCw, ShieldCheck } from "lucide-react";
...
export function ScenarioLab() {
  ...
  return (
    <div className="workspace sx-workspace">
      ...
      {scenario && forecast && <section className="sx-panel"><div className="sx-heading"><div><span>{copy.result.index}</span><h2>{scenario.name}</h2></div><strong>{scenario.status === "completed" ? "SIMULACIÓN COMPLETADA" : scenario.status}</strong></div><ScenarioSummary scenario={scenario} /><div className="fx-context-notice"><BrainCircuit size={15} /><span><strong>{ui.decisionCenter.links.fromScenario}</strong>El análisis conservará este escenario como hipótesis separada del forecast oficial.</span><Link href={`/decision-center?forecast_run_id=${scenario.forecast_run_id}&scenario_run_id=${scenario.id}`}>{ui.decisionCenter.links.fromScenario}</Link></div><ScenarioChart forecast={forecast} scenario={scenario} /><div className="sx-assumption-ledger">{scenario.assumptions.map((item) => <article key={item.id}><span>{item.order_index + 1}</span><div><strong>{item.label}</strong><small>{item.source_type === "historical_evidence" ? copy.builder.evidenceBadge : copy.builder.manualBadge} · {item.start_at.slice(0, 10)} → {(item.end_at ?? item.start_at).slice(0, 10)}</small></div><b>{item.unit === "absolute" ? item.magnitude : `${((item.magnitude ?? 0) * 100).toFixed(1)}%`}</b></article>)}</div></section>}
