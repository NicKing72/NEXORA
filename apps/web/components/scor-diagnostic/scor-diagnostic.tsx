  ...
  useEffect(() => {
    ...
    Promise.all([listScorDefinitions(), listScorAssessments(), listScorProfiles()])
      .then(async ([definitionItems, assessmentItems, profileItems]) => {
        ...
        if (assessmentItems[0]) {
          const detail = await getScorAssessment(assessmentItems[0].id);
  ...
  return (
    <div className="workspace sd-workspace">
      ...
      <section className="sd-panel">
        <div className="sd-heading"><div><span>{copy.scope.index}</span><h2>{copy.scope.title}</h2></div><div className="sd-actions"><button type="button" onClick={() => setFormOpen(true)}><Plus size={15} />{copy.scope.new}</button><button type="button" disabled={working} onClick={() => void regenerateDemo()}><Database size={15} />{copy.scope.demo}</button><button className="sd-primary" type="button" disabled={working || !assessment} onClick={() => void calculate()}><Calculator size={15} />{working ? copy.scope.calculating : copy.scope.calculate}</button></div></div>
